--[[
    补间动画系统 (Tween)
    从 zhanshen 项目移植
    基于 kikito/tween.lua (MIT License)
    
    使用示例:
    ```lua
    local obj = { x = 0, y = 0 }
    local tween = Tween.New(2, obj, { x = 100, y = 200 }, 'outQuad')
    
    -- 在计时器中更新
    Timers:CreateTimer(0.01, function()
        local complete = tween:update(0.01)
        if complete then
            print('动画完成!')
            return nil
        end
    ...
    - inQuart, outQuart, inOutQuart (四次)
    - inQuint, outQuint, inOutQuint (五次)
    - inSine, outSine, inOutSine (正弦)
    - inExpo, outExpo, inOutExpo (指数)
    - inCirc, outCirc, inOutCirc (圆形)
    - inElastic, outElastic, inOutElastic (弹性)
    - inBack, outBack, inOutBack (后退)
...
GameRules.Tween = GameRules.Tween or Tween
