...
export class TrainingRoomManager {
    ...
    private static GenerateRoom(index: number) {
        ...
        if (config.name === "天枢界域") {
             ...
             for (let r = 0; r < rows; r++) {
                 for (let c = 0; c < cols; c++) {
                     ...
                     const tile = CreateUnitByName("npc_dota_dummy_prop_custom", Vector(tx, ty, center.z - 110), false, undefined, undefined, DotaTeam.NOTEAM);
                     if (tile) {
            ...
            // Keep the "Massive Single Floor" for others
             const floor = CreateUnitByName("npc_dota_dummy_prop_custom", Vector(center.x, center.y, center.z - 250), false, undefined, undefined, DotaTeam.NOTEAM);
             if (floor) {
        ...
        if (config.name === "天枢界域") {
             ...
             if (house) {
                 ...
                 const p1 = ParticleManager.CreateParticle("particles/units/heroes/hero_oracle/oracle_fates_edict.vpcf", ParticleAttachment.ABSORIGIN_FOLLOW, house);
                 const p2 = ParticleManager.CreateParticle("particles/units/heroes/hero_tinker/tinker_defense_matrix.vpcf", ParticleAttachment.ABSORIGIN_FOLLOW, house);
             }
             ...
             for (let i = 0; i < numEdge; i++) {
                ...
                if (i % 2 == 0) {
                     const crystal = CreateUnitByName("npc_dota_dummy_prop_custom", Vector(ex, ey, center.z + 50), false, undefined, undefined, DotaTeam.NOTEAM);
                     if (crystal) {
            ...
            for (let i = 0; i < numEdge; i++) {
                ...
                if (i % 3 == 0 && (config as any).pillarModel) {
                     const pillar = CreateUnitByName("npc_dota_dummy_prop_custom", Vector(ex, ey, center.z), false, undefined, undefined, DotaTeam.NOTEAM);
                     if (pillar) {
                ...
                const p = ParticleManager.CreateParticle("particles/units/heroes/hero_cloud/cloud_base_smoke.vpcf", ParticleAttachment.WORLDORIGIN, undefined);
    ...
    private static SpawnCreeps(center: Vector, roomName: string) {
        ...
        for (let i = 0; i < necCount; i++) {
            const unit = CreateUnitByName("npc_dota_necronomicon_warrior_1", Vector(center.x + 200, center.y + 200, center.z), true, undefined, undefined, team);
             if (unit) {
                unit.SetAbsAngles(0, RandomFloat(0, 360), 0);
            }
        }
